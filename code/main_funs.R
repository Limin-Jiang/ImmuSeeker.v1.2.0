...
getUnique = function(subd1){
  ...
  data_temp = data.frame(HLAname = geneName, Num = length(unique(subd1$reads)),Class = "Gene", Genes = geneName,  Total = length(unique(subd1$reads)), stringsAsFactors = FALSE)
  ...
  for (Class1_list_ID in Class1_list){
    ...
    data_temp = data.frame(HLAname = Class1_list_ID, Num =  length(unique(subsubd1$reads )), Class = "One-field", Genes = geneName, Total = length(unique(subsubset2$reads)),stringsAsFactors = FALSE)
    data_all = rbind(data_all,data_temp)
    ...
    if (nrow(subsubd1) > 0){
      ...
      for (ii in Class2_list_temp){
        ...
        data_temp = rbind(data_temp,data.frame(HLAname = ii, Num = length(aa),Total = length(unique(subsubset2$reads)),stringsAsFactors = FALSE))
      }
      if (nrow(data_temp) > 0){

...
if (nrow(d_cov) == 0){
  ...
  for (geneName in geneNameS){
    ...
    if (nrow(subd1) > 0){
      ...
      data_TEMP$allele = sapply(strsplit(data_TEMP$AlleleID, split=':', fixed=TRUE), function(x) (x[1]))
      ...
      alleles_list = alleles_list$AlleleID
      if (length(aa)>0){
  ...
  data_end = data_end[,-6]
  colnames(data_end) = c("AlleleID","Num.Unique" , "Level","Gene", "Total"  )
  colnames(data_all_out)= c("AlleleID","Num.Unique" , "Level","Gene", "Total"  )
}
...
if (nrow(data_end) > 0){
  ...
  if (nrow(data_end) > 0){
    ...
    if (expression_lag == 'true'){
      ...
      rm(Newbam)
      data_all_out = merge(data_all_out,d_gene_length,by.x = "AlleleID",by.y = "name",all.x = TRUE)
      
      for (ii in 1:nrow(data_all_out)){
        HLA_idx = data_all_out$AlleleID[ii]
    ...
    idx = match(data_end$AlleleID,data_all_out$AlleleID)
